// ═══════════════════════════════════════════════════════════
// 🎧 PODCASTS RSS — SERVICIO (F5.1 · Medios)
// El reproductor de podcasts "de verdad" (novelas, audiolibros,
// ciencia ficción): listas públicas de episodios vía RSS, sin
// cuenta y sin pago. Puerto del RiderTrack V2 (F3.43) a FitTrack,
// adaptado: persistencia SOLO local (sin Firestore, sin voz-nav).
...
//     `ft_pod_subs_{uid}` / `ft_pod_pos_{uid}` (instantáneo,
//     SOLO en el teléfono — cero Firestore, cero reglas nuevas
//     y cero cambios en el bot).
...
const EVENTO = 'ft-podcasts-cambio';
const NOMBRE_CACHE = 'ft-podcasts-v1';
const CLAVE_SUBS = (uid: string) => `ft_pod_subs_${uid}`;
const CLAVE_POS = (uid: string) => `ft_pod_pos_${uid}`;
const CLAVE_VEL = 'ft_pod_velocidad';
const CLAVE_FEED = (uid: string, url: string) => `ft_pod_feed_${uid}_${hashUrl(url)}`;
...
export function pausarEpisodioRSS(): void {
...
/**
 * Arranca el servicio para este usuario (estado local al toque:
 * suscripciones, posiciones y velocidad del teléfono).
 */
